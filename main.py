...
from data.level_module_task import Module
...
@app.route('/solv/<int:lvl><int:module><int:ind>', methods=['GET', 'POST'])
def solving_eq(lvl, module, ind):
    if request.method == 'POST':
        ...
        # db_sess = db_session.create_session()
        # eq = db_sess.query(Module).filter(Module.level_id == lvl)[module][ind]
        eq = gen_equations.gen_eq(12)
