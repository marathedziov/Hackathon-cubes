...
def gen_lvl3():
    ...
    a2, b2 = 1, a1
    c2 = x + b2 * y
    eq_1 = ['x.png'] * a1 + ['y.png'] * b1 + [str(c1)]
    eq_2 = ['x.png'] * a2 + ['y.png'] * b2 + [str(c2)]
    lst = ' '.join(eq_1) + ':' + ' '.join(eq_2)
...
def gen_message(res):
    if res:
        return choice(["Ты настоящий нейронный гений!",
                       ...
                       "Ты прям мозг!"])
    return choice([
        "Ой-ой, мимо кассы! Попробуйте ещё раз – я верю в вас!",
        "Не совсем то, но я ценю ваш энтузиазм. Попробуйте снова!",
        "Упс, промах. Как говорится, учимся на ошибках!",
        "Ну, это было неожиданно... но неправильно! Давайте ещё!",
        "Не то пальто. Может, попробовать другой угол зрения?",
        "Кажется, это не то, что я ожидал. Ещё одна попытка?",
        "Хмм... близко, но мимо. Настоящая победа ещё впереди!",
        "А вот и ошибка. Но ничего страшного – вы на верном пути!",
        "Упс! Сдаётся мне, что правильный ответ где-то рядом. Пробуем ещё?",
        "Эх, это был промах. Но каждый промах приближает к цели, верно?",
        "Нет-нет, не сдавайтесь! Возможно, нужен другой подход?",
        "Кхм, это не то, но зато весело. Попробуйте ещё раз!",
